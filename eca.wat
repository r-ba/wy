...
(module
  ...
  (func (export "update") (param $canvasWidth i32) (param $canvasHeight i32) (param $rule i32)
    (local $x0 i32)
    (local $x1 i32)
    (local $x2 i32)
    (local $x3 i32)
    ...
    (set_local $x2 (i32.const 1))
    (set_local $x3 (i32.const 1))
    (set_local $x4 (i32.const 1))
    (set_local $x5 (i32.const 1))
    (set_local $x6 (i32.const 1))
    (set_local $x7 (i32.const 1))

    (set_local $tmp (call $bin (local.get $rule) (i32.const 128)))
    (if (i32.ne (local.get $tmp) (local.get $rule))
      (set_local $rule (local.get $tmp))
      (set_local $x7 (i32.const 0)))
    (set_local $tmp (call $bin (local.get $rule) (i32.const 64)))
    (if (i32.ne (local.get $tmp) (local.get $rule))
      (set_local $rule (local.get $tmp))
      (set_local $x6 (i32.const 0)))
    (set_local $tmp (call $bin (local.get $rule) (i32.const 32)))
    (if (i32.ne (local.get $tmp) (local.get $rule))
      (set_local $rule (local.get $tmp))
      (set_local $x5 (i32.const 0)))
    (set_local $tmp (call $bin (local.get $rule) (i32.const 16)))
    (if (i32.ne (local.get $tmp) (local.get $rule))
      (set_local $rule (local.get $tmp))
      (set_local $x4 (i32.const 0)))
    (set_local $tmp (call $bin (local.get $rule) (i32.const 8)))
    (if (i32.ne (local.get $tmp) (local.get $rule))
      (set_local $rule (local.get $tmp))
      (set_local $x3 (i32.const 0)))
    (set_local $tmp (call $bin (local.get $rule) (i32.const 4)))
    (if (i32.ne (local.get $tmp) (local.get $rule))
      (set_local $rule (local.get $tmp))
      (set_local $x2 (i32.const 0)))
    (set_local $tmp (call $bin (local.get $rule) (i32.const 2)))
    (if (i32.ne (local.get $tmp) (local.get $rule))
      (set_local $rule (local.get $tmp))
      (set_local $x1 (i32.const 0)))
    (set_local $tmp (call $bin (local.get $rule) (i32.const 1)))
    (if (i32.ne (local.get $tmp) (local.get $rule))
      (set_local $rule (local.get $tmp))
    ...
    (call $updateSystem
      (local.get $x0) (local.get $x1) (local.get $x2) (local.get $x3)
      (local.get $x4) (local.get $x5) (local.get $x6) (local.get $x7)
      (local.get $canvasWidth) (local.get $canvasHeight))
    (call $updateCanvas (local.get $canvasWidth) (local.get $canvasHeight))
  )
